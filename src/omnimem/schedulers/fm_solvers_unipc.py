...
class FlowUniPCMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    @property
    def step_index(self):
        """Current timestep index counter."""
        return self._step_index

    @property
    def begin_index(self):
        """Index of the first timestep; set via set_begin_index before inference."""
        return self._begin_index

    def set_begin_index(self, begin_index: int = 0):
        """Set the begin index before inference.

        Args:
            begin_index: The starting step index.
        """
    ...
    def _threshold_sample(self, sample: torch.Tensor) -> torch.Tensor:
        """Apply dynamic thresholding to prevent pixel saturation (arxiv:2205.11487)."""
        dtype = sample.dtype
        ...
        # flatten sample for quantile calculation along each image
    ...
    def add_noise(
        ...
        if shift is not None:
            alphas = np.linspace(1, 1 / self.config.num_train_timesteps,
                             self.config.num_train_timesteps)[::-1].copy()
            sigmas = 1.0 - alphas
            sigmas = torch.from_numpy(sigmas).to(dtype=torch.float32)
