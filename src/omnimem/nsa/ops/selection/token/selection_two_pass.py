...
Two backward strategies for dkv:
  'inverted'  — build inverted index (counting sort), iterate only active queries
  'mask'      — build dense block_mask, tile-skip with vectorized matmul
  'auto'      — pick based on sparsity ratio T/NS (default)
"""
import math
import torch
import triton
...
from .select_fwd import _sel_attn_fwd_kernel

# Device-adaptive autotune configs
...
if _sm_version >= 90:
    _sel_attn_bwd_preprocess_configs = [
        ...
        triton.Config({'BLOCK_M': 64}, num_warps=base_num_warps * 2, num_stages=3, num_ctas=1),
    ]
    ...
    _sel_attn_bwd_dkv_mask_configs = [
        # BLOCK_Q=16: safe fallback for large PADDED_BLOCK_SIZE (e.g. 64) + large DP (128)
        triton.Config({'BLOCK_Q': 16}, num_warps=base_num_warps, num_stages=2),
        triton.Config({'BLOCK_Q': 16}, num_warps=base_num_warps * 2, num_stages=2),
        triton.Config({'BLOCK_Q': 32}, num_warps=base_num_warps * 2, num_stages=2),
        triton.Config({'BLOCK_Q': 32}, num_warps=base_num_warps * 2, num_stages=3),
        triton.Config({'BLOCK_Q': 64}, num_warps=base_num_warps * 2, num_stages=1),
    ...
    _sel_attn_bwd_dkv_mask_configs = [
        triton.Config({'BLOCK_Q': 16}, num_warps=base_num_warps, num_stages=1),
...
def _inverted_index_histogram_kernel(
        TopIdx,          # [B, M, H, T]
        Histogram,       # [B, H, NS]  int32, zero-initialized
    ...
    """Count (query, topk-slot) pairs per KV block; IS_CAUSAL filters k_chunk <= q_chunk."""
    i_b = tl.program_id(0)
    i_h = tl.program_id(1)
...
def _inverted_index_fused_kernel(
        TopIdx,          # [B, M, H, T]
        WritePos,        # [B, H, NS]  int32 — initialized to CSR offsets (atomic counter)
        SortedQueries,   # [B, H, M*T] int32 — output
        stride_tb, stride_tm, stride_th, stride_tt,
    ...
    """Scatter query indices into CSR buckets; same causal filter as histogram kernel."""
    i_b = tl.program_id(0)
    i_h = tl.program_id(1)
    i_m_blk = tl.program_id(2)
...
def build_inverted_index(top_idx, NS, selection_block_size=1, chunk_size=1, causal=False):
    """
    Build inverted index via counting sort (O(M*T + NS)).
    causal=True pre-filters pairs so dkv kernel needs no causal branching.

    Returns:
        sorted_queries:  [B, H, M*T]  int32 — query indices grouped by block id
        block_offsets:   [B, H, NS+1] int32 — CSR offsets per KV block
    """
...
def build_inverted_index_torch(top_idx, NS):
    """Reference via PyTorch sort + searchsorted; O(M*T*log). Kept for validation."""
    B, M, H, T = top_idx.shape
    device = top_idx.device

    block_flat = top_idx.permute(0, 2, 1, 3).reshape(B, H, M * T)
    query_ids = (
    ...
    sorted_blocks, perm = block_flat.sort(dim=-1, stable=True)
    sorted_queries = query_ids.gather(-1, perm).to(torch.int32)

    boundaries = torch.arange(NS + 1, device=device, dtype=sorted_blocks.dtype)
    boundaries = boundaries.view(1, 1, -1).expand(B, H, -1).contiguous()
    block_offsets = torch.searchsorted(sorted_blocks.contiguous(), boundaries)

    return sorted_queries.contiguous(), block_offsets.to(torch.int32).contiguous()
...
def _sel_attn_bwd_dq_kernel(
        ...
        SELECTION_BLOCK_SIZE: tl.constexpr, PADDED_BLOCK_SIZE: tl.constexpr,
    ...
    do_base = DOut + b * stride_ob + m * stride_om + h * stride_oh
    d_base = Delta + b * stride_lb + m * stride_lm + h * stride_lh
    dq_base = DQ + b * stride_qb + m * stride_qm + h * stride_qh

    # offsets & masks
    offs_d = tl.arange(0, DP)
    mask_d = offs_d < D
    offs_p = tl.arange(0, PADDED_BLOCK_SIZE)
    mask_valid = offs_p < SELECTION_BLOCK_SIZE
    offs_1 = tl.arange(0, 1)

    # load Q, dO tokens: [1, D]; LSE, delta: [1]
    q_blck = tl.load(q_base + offs_d[None, :] * stride_qd, mask=mask_d[None, :], other=0.0).to(tl.float32)
    do_blck = tl.load(do_base + offs_d[None, :] * stride_od, mask=mask_d[None, :], other=0.0).to(tl.float32)
    l_blck = tl.load(l_base + offs_1 * stride_lh)
    d_blck = tl.load(d_base + offs_1 * stride_lh)
    ...
    for idx in range(T):
        ...
        if not causal or (col // CHUNK_SIZE <= q_chunk_idx and col >= 0):
            ...
            accum = tl.dot(ds, k_blck, acc=accum, allow_tf32=ALLOW_TF32)  # dQ += ds @ K

    # epilogue: write dQ
    dq_ptrs = dq_base + offs_d[None, :] * stride_qd
    tl.store(dq_ptrs, accum, mask=mask_d[None, :])


@triton.autotune(
    configs=_sel_attn_bwd_dkv_mask_configs,
    key=['M', 'D', 'SELECTION_BLOCK_SIZE'],
)
@triton.jit
def _sel_attn_bwd_dkv_mask_kernel(
    ...
    # KV block columns for this program
    col_start = i_s * SELECTION_BLOCK_SIZE
    offs_d = tl.arange(0, DP)
    mask_d = offs_d < D
    offs_p = tl.arange(0, PADDED_BLOCK_SIZE)
    mask_valid = offs_p < SELECTION_BLOCK_SIZE
    cols = col_start + offs_p
    mask_n = cols < N
    kv_mask = mask_n[:, None] & mask_valid[:, None] & mask_d[None, :]

    k_base = K + b * stride_kb + h * stride_kh
    v_base = V + b * stride_vb + h * stride_vh

    # load K, V blocks: [BLOCK_SIZE, D]
    ...
    b_dk = tl.zeros([PADDED_BLOCK_SIZE, DP], dtype=tl.float32)
    b_dv = tl.zeros([PADDED_BLOCK_SIZE, DP], dtype=tl.float32)
    log_scale = softmax_scale * 1.44269504

    # iterate over Q tiles; use block_mask to skip inactive rows
    ...
    for m_start in range(0, M, BLOCK_Q):
        ...
        if tl.sum(b_mask.to(tl.int32)) > 0:
            ...
            # QK^T -> scores: [BLOCK_Q, BLOCK_SIZE]; recompute softmax weights
            qk = tl.dot(q_blck, tl.trans(k_blck), allow_tf32=ALLOW_TF32) * log_scale

            base_mask = mask_valid[None, :] & mask_n[None, :]
            if causal:
            ...
            exp_qk = tl.math.exp2(qk - l2[:, None])
            exp_qk = tl.where(b_mask[:, None] & mask_m[:, None] & (l_vals[:, None] > -1e6), exp_qk, 0.0)
    ...
    write_mask = mask_n[:, None] & mask_valid[:, None] & mask_d[None, :]
...
# dKV kernel: KV-major, inverted-index path (no atomics)
@triton.autotune(
    configs=_sel_attn_bwd_dkv_inv_configs,
    key=['D', 'SELECTION_BLOCK_SIZE'],
)
@triton.jit
def _sel_attn_bwd_dkv_inv_kernel(
        Q, K, V, Lse, DOut, Delta,
        SortedQueries,   # [B, H, M*T] int32
        BlockOffsets,     # [B, H, NS+1] int32
        softmax_scale: tl.constexpr, causal: tl.constexpr,
        DK, DV,
        stride_qb, stride_qh, stride_qm, stride_qd,
        stride_kb, stride_kh, stride_kn, stride_kd,
    ...
    # CSR range for this KV block from the inverted index
    ...
    for i_tile in range(num_tiles):
        ...
        base_mask = mask_valid[None, :] & mask_n[None, :] & mask_q[:, None]
        if causal:
            # Needed when SELECTION_BLOCK_SIZE > CHUNK_SIZE (block spans multiple chunks).
        ...
        dp = tl.dot(do_blck, tl.trans(v_blck), allow_tf32=ALLOW_TF32)  # dp = DO @ V^T
        ds = exp_qk * (dp - d_vals[:, None]) * softmax_scale

        b_dk += tl.dot(tl.trans(ds), q_blck, allow_tf32=ALLOW_TF32)  # dK += ds^T @ Q
...
class SelectionAttention2p(torch.autograd.Function):
    ...
    def forward(
        ...
        OFFSET_M = 0
        padded_block_size = triton.next_power_of_2(selection_block_size)
        grid = (B, M - OFFSET_M, H)
        ...
        # inverted wins when sparse (ratio < 0.5) and large enough (n > 200K or num_blocks > 256)
        if bwd_method == 'auto':
            ratio = T / num_blocks
            n = M * T
        ...
        ctx.padded_block_size = padded_block_size
    ...
    def backward(ctx, *grad_outputs):
        ...
        padded_block_size = ctx.padded_block_size
        ...
        _sel_attn_bwd_dq_kernel[(B, M - OFFSET_M, H)](
        ...
        return dq, dk.to(k.dtype), dv.to(v.dtype), None, None, None, None, None, None, None, None